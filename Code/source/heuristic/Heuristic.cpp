...
void Heuristic::preparation()
{
    ...
    // Verificar distancia de deposito mais próximo de cliente mais distante
...
{
    // Metodo da adição mais barata com torneio para AG
    int demand = 0;
    int num, p, s, r, x, y;
...
void Heuristic::sdm(int percent)
{
    ...
    pij = Helper::initializeIntegerMatriz(grafo->getVerticeCount(), grafo->getBssCount()); // Matriz de subconjunto dominante mínimo (BSSsxClientes)
    vi = new int[grafo->getBssCount()]; // Vetor de número de adjacentes BSSs
        ...
        for(int j = 1; j < (int)solution[i].size(); j++)
        {
            ...
            else
            {
                ...
                if(!add)
                {
                    ...
                    if(!add)
                    {
                        cout << endl << endl << "BSS - Solução Inviável!!" << endl;
                        this->qtRotas = solution.size()+1;
                        return;
                    }
                }
...
void Heuristic::plot()
{
    ...
    solucao.open(outputFile.c_str());
    //solucao << "cd 'C:/Users/Bráulio/Dropbox/Projeto_TCC/1.9/results'" << endl;
